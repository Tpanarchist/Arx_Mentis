...
behavior is retroactively declared semantic law.
...
The installable package contains no syntax, AST, semantics, evaluator, runtime
values, or IR. Naming a future boundary does not select its representation.

## Non-goals

The provisional foundation is not a frozen syntax or semantic specification. The
bootstrap does not select concrete representation, type/value discipline, integer
behavior, general equality, evaluation order, mutability, error model, module
system, concurrency model, memory model, IR shape, or backend target. It also does
not publish a package or promise that the `arx-mentis` name is available on PyPI.
